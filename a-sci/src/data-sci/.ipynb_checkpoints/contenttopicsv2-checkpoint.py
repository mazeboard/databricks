...
content_clusters = data_project.extract_layers(configuration)
sample_count = configuration['compute_configuration']['value']['sampling']
prompt = configuration['compute_configuration']['value']['prompt']

(content_clusters, sample_count, prompt)

# COMMAND ----------

embeddings = spark.sql(f"""SELECT * FROM {context["table_prefix"]}.comp_content_embeddings WHERE project_id='{context["project_id"]}' AND content_group='{context["import_id"]}'""")

display(embeddings.limit(50))
...
content_topic_layers = [
    spark.createDataFrame(compute_content_topics(embeddings, content_clusters, index, clustered_content)).select(
...
sample_udf = udf(lambda ids:  random.sample(ids, sample_count), ArrayType(StringType()))
...
broadcasted_prompt = sc.broadcast(prompt)

def summarize_fn():
    from llama_cpp import Llama
    llm = Llama.from_pretrained(
        repo_id="bartowski/Phi-3.1-mini-128k-instruct-GGUF",
        filename="Phi-3.1-mini-128k-instruct-Q6_K_L.gguf",
        verbose=False, 
        n_ctx=65536,
        seed=1048576,
    )
    ...
    def process_content(content):
        prompt = f"""<|system|>You are a helpful assistant.<|end|>
            <|user|>{broadcasted_prompt.value}{content}<|end|>
            <|assistant|>"""
...
spark.sql(f"""
    DELETE FROM {context["table_prefix"]}.comp_content_topics
    WHERE project_id='{context["project_id"]}' AND
          content_group='{context["import_id"]}'
""")
...
topics.writeTo(f"""{context["table_prefix"]}.comp_content_topics""").using("delta").append()
...
display(embeddings_topics)

# COMMAND ----------

display(pd.DataFrame(embeddings.toPandas()['dense'].tolist()))
...
def compute_scale(clusters: MutableSequence[int], previous: int, current: int) -> (int, MutableSequence[int]):
    scaled = previous * pow(10, len(f"{current}"))
    clusters.append(previous)
    return (scaled, clusters)

clusters_scale = functools.reduce(lambda a, e: compute_scale(a[1], a[0], e), content_clusters[::-1], (1, []))[1][::-1]

prepare = pd.concat(
    [
        pd.DataFrame(embeddings.toPandas()['dense'].tolist()),
        pd.DataFrame(
            np.transpose( clustered_content[content_clusters, :]),
            columns=[f'k{content_cluster}' for content_cluster in content_clusters]
        ) + 1 * clusters_scale
...
def compute_positions():
    ...
    result = pd.DataFrame(pacmap.PaCMAP(n_components=2, n_neighbors=5, MN_ratio=0.5, FP_ratio=50).fit_transform(prepare))
    result['content_id'] = embeddings.toPandas()['content_id']

    return result

content_positions = spark.createDataFrame(compute_positions())

display(content_positions)
